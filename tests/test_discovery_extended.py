...
def test_node_text():
    dom = minidom.parseString("<root><empty></empty><present>hello</present></root>")
    assert _node_text(dom, "missing", default="def") == "def"
    assert _node_text(dom, "empty", default="def") == "def"
    assert _node_text(dom, "present") == "hello"
...
async def test_find_gateways():
    ...
    with patch('asyncio.get_running_loop') as mock_loop, \
         patch('OWNd.discovery._get_scpd_details', return_value={"modelName": "F454", "port": 20000}) as mock_scpd, \
